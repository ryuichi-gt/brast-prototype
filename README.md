...
- **週次ブリーフィング（中核画面）**:
  ...
  - 右の思考プロセス（エージェント9工程のパイプライン）
  - 下のフィードバック dock
  - **差し戻し → トレースバック → 下流再実行**: スライドの「直す」やフィードバック入力で、
    原因のエージェント工程をハイライトし、そこから下流（スコアゲートまで）だけを再実行できる状態にする。
    承認済みの成果物は残す挙動。
  - スライド送り（矢印 / ドット / キーボード ← →）、承認アクション。
...
- **レスポンシブ**: PC / タブレット / スマホ対応。狭幅では思考プロセスとdockがデッキの下にスタック。
