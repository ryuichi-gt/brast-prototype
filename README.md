...
既定テナント **JRCエンジニアリング 医療ITソリューション事業部** は、公開情報で裏取りした
実データでブラッシュアップ済み（2026-06 リサーチ）。

- **会社**: JRCエンジニアリング＝日本無線（JRC）グループ／日清紡HD傘下、ISO 9001・27001。
  医療IT事業は **ORCA中心**（ORCA導入・電子カルテ連携・オンライン資格確認・レセボット・Medical RPA・DICOM）。
  2024年に子会社 **メディカル・オーシャン**（医療DX対応）設立。公式 X/LinkedIn は確認できず＝「価値を発信できていない」前提に合致。
- **実トレンド**: 電子カルテ情報共有サービス（三文書六情報・HL7 FHIR、本格運用 2026年度冬）/ 医療DX推進体制整備加算（要件段階強化）/ 安全管理GL第6.0版（2027年度 二要素認証・BCP）/ 電子処方箋 病院導入<20% / 大阪のランサム被害（10億円和解）。
- **実競合**: ウィーメックス（旧PHC・メディコム）/ NEC（MegaOak/iS）/ 富士通Japan（デジタルホスピタル）/ トレンドマイクロ・TXOne。空白＝**保守・運用×マルチベンダー移行の現実**。
- **生成記事の中身**: BRAST の Content エージェントが生成する想定の**フル本文**を 7 本収録（記事/SNS草稿/配布資料）。
  メディアプランや「コンテンツ」一覧のタイトルをタップ → **記事リーダー**で本文・4体レビューの内訳・**根拠ソース（実URL）**まで確認できる。
- **数値の扱い**: スコア・リーチ・関連度は実測値ではなく**現実的な推定値**（Phase 1 規約）。本番では Phase 2 のアナリティクス/スコアゲートに置き換える。出典は各記事の「根拠ソース」を参照。
- 他テナント（SympaFit / ツズリーBCP / Libero）は引き続きダミーデータ。
